(ns snomed-ct.main
  "Kotodama WASM entrypoint for the snomed_ct clean-room actor (L4) — Clojure port.
  ...
  facts (`snomed_ct.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "snomed_ct")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"snomed_ct.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "snomed_ct-compat" :tier tier :entities entities} 200])
...
(defn start! [] :snomed_ct-compat/ready)
